...
function render_seo(array $profile, string $title, string $description, string $path = '/', string $type = 'website'): void
{
    ...
    $person = ['@context'=>'https://schema.org','@graph'=>[
        ...
        ['@type'=>'Person','@id'=>$base . '/#person','name'=>$name,'jobTitle'=>$profile['role_title'] ?? 'Full-Stack Web Developer','description'=>$profile['tagline'] ?? $description,'url'=>$base . '/','sameAs'=>array_values(array_filter([$profile['linkedin'] ?? '', $profile['github'] ?? '']))],
